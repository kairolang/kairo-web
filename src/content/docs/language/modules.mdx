...
Kairo's module system maps source files and directories to namespaces. Each `.kro` file is a module.
A directory containing a `.kro` file with the same name as the directory is a library its entry
...
Each `.kro` file is a module named after the file (without the extension). A directory with an
...
project/
  main.kro
  math/
    math.kro        <- library entry for "math"
    vector.kro
    matrix.kro
    internal/
      helpers.kro
...
```kairo
// main.kro
import math              // imports the math library (math/math.kro)
import math::vector      // imports math/vector.kro specifically
import math::matrix      // imports math/matrix.kro specifically
```

The library entry file (`math/math.kro`) controls what the library exports. Files in the directory
...
compression::codec::encode_base64(data)   // defined in encoding.kro
compression::codec::compress(data)         // defined in compression.kro
```
...
```kairo
// a.kro
import b        // compile error if b.kro imports a
...
A directory with a `.kro` file matching the directory name acts as a library. The entry file
controls exports:

```kairo
// network/network.kro (library entry)
import tcp       // network/tcp.kro
import udp       // network/udp.kro
priv import dns  // network/dns.kro not re-exported
...
network::connect("localhost", 8080)   // from network.kro
network::tcp::listen(8080)            // from network/tcp.kro
