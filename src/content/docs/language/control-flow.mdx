...
Kairo has no ternary `? :` operator. Use `if`/`else` as an expression instead the last expression in each
branch is the result value:
...
`switch` is Kairo's value-based multi-way branch. It operates on concrete values integers, enums, strings
without destructuring or pattern guards.
...
switch status_code {
    case 200 {
        ...
        std::println(f"Unknown: {status_code}")
    }
}
```
...
`switch` is exhaustive. The compiler emits an error if not all possible values of the switched type are
covered. The `default` case covers any remaining values.

For enums, the compiler knows the full set of variants and can verify coverage without `default`:
...
The `.Variant` shorthand syntax is available when the compiler can infer the enum type from the switch
operand. The fully qualified form `Direction::North` also works.

See [Enums](/docs/language/enums) for enum declarations.

### Fall-through

Cases break implicitly no `break` statement needed. This is the opposite of C/C++ behavior. To opt into
C-style fall-through, annotate the entire `switch` with `@fallthrough`:

```kairo
@fallthrough switch priority {
    case 1 {
        handle_critical()
...
`@fallthrough` applies to the entire switch statement, not individual cases. Without the attribute, each case
is an isolated block.
...
> [!IMPORTANT]
> `match` is planned but not yet implemented. Use `switch` for value-based dispatch in the meantime.
...
| `switch` fall-through (implicit) | `@fallthrough` attribute (opt-in) |
