...
Safe pointers can be null. `&null` is the null pointer literal, equivalent to `nullptr` in C++:

```kairo
var p: *i32 = &null
```

Dereferencing a null `*T` panics in both debug and release builds. The compiler inserts a null check
before every dereference:

```kairo
var p: *i32 = &null
*p = 10   // panic: null pointer dereference
```

When the compiler can prove a pointer is non-null (e.g., it was just assigned from `&x` where `x` is
a live variable), the null check is elided. The safety guarantee is the same the optimization is
invisible to the programmer.

### Explicit null checking

Check for null with a direct comparison before dereferencing:

```kairo
var p: *i32 = get_pointer()
if p != &null {
    std::println(*p)   // compiler knows p is non-null here, check elided
}
```
...
Pointers and nullable types (`T?`) are separate systems:

| | `*T` | `T?` |
|---|---|---|
| Can be null | Yes (`&null`) | Yes (`null`) |
| Null check mechanism | `ptr != &null` | `val?`, `?.`, `??`, `unwrap!()` |
| Dereference null | Panics | Compile error (must check first) |
| AMT tracked | Yes | N/A (value type) |

Pointers do not support `?`, `?.`, `??`, or `unwrap!()`. Nullable types do not support `->` or
pointer arithmetic.

See [Variables](/docs/language/variables#nullable-types) for the nullable type system.
