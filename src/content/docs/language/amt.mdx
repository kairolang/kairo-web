...
When AMT promotes a pointer to a smart pointer, the destructor fires at the end of the **lexical
scope** of the binding that owns the pointer matching the behavior of non-promoted variables.
...
// cfg's destructor runs here, at the closing brace
// If AMT promoted cfg to Unique, the Unique destructor runs here
// If AMT promoted cfg to Shared, the refcount decrements here
```

There is no drop-at-last-use optimization. Destruction is deterministic and tied to lexical scope.
This means:

- Side effects in destructors (file close, lock release, network disconnect) are predictable.
- `finally` blocks and destructor ordering compose correctly.
- The programmer can reason about destruction order by reading the source.
