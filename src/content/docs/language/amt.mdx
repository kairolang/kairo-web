...
fn process() {
    ...
    fill(data)
    consume(data)
    std::destroy(data)    // fully contained -> plain *T
}
```
...
fn make() -> *i32 {
    var x = 42
    return &x
    // debug: rewrite `var x = 42` into `var x = std::create<i32>(); *x = 42`,
...
error[AMT]: address of stack local 'x' escapes its scope
  ...
  note: in debug, AMT would heap-promote: var x = std::create<i32>()
...
The default for `std::create<T>()` and all standard-library heap operations. Overridable with
...
A scoped-allocator variant for embedded and bare-metal targets, conforming through `static` functions only no indirection, no vtable. The compiler replaces `std::create<T>()` with a direct static call plus placement construction. AMT treats freestanding allocators identically to scoped allocators for lifetime tracking; the difference is a codegen concern.
...
**Trivial destructors** (no observable side effect just memory to reclaim) run at **last use**. AMT places the `std::destroy` as early as the final use permits, since reclaiming memory after the last use is indistinguishable from reclaiming it at the scope brace:

```kairo
var foo = std::create<SomeClass>()    // trivial destructor
use(foo)
// std::destroy(foo) inserted here
std::println(10)
```
...
```kairo
fn example() {
    var cfg = std::create<Config>(8080)    // side-effecting destructor
    use(cfg)
    std::println(10)
}
