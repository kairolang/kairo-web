...
`#include "file.kro"` work transparently in C++ translation units.
...
the included file has a `.kro` extension, `kcc`:
...
2. Compiles the `.kro` file into an object file.
...
```sh
kairo my_code.kro -c -o my_code -xc++ -header my_code.hh
