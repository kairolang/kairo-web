...
Because UTF-8 is a variable-width encoding, indexing by codepoint (`s[i]`) requires a linear scan and is O(n).
Use `.bytes` for O(1) byte-level access when performance matters:
...
`*T` is a thin pointer (8 bytes). It is non-null by construction and supports pointer arithmetic when the
compiler can track its provenance via [AMT](/docs/language/amt). Dereferencing a `*T` emits a null check.

`unsafe *T` is a raw C-style pointer with no compiler tracking. It can be null, and dereferencing a null
`unsafe *T` is undefined behavior. Use `unsafe *T` for [C/C++ interop](/docs/language/c-c++), custom
allocators, and other low-level scenarios. See [Pointers](/docs/language/pointers) and
[Unsafe](/docs/language/unsafe) for full details.
