...
offset and offending bytes. A BOM at the start of a `.k` file is an error.
...
- **Codepoint indexing:** `s[i]` returns a `char` (4-byte Unicode scalar value)
  at codepoint position `i`. This is O(1) amortized -- the implementation uses
  a sparse breadcrumb cache to locate the nearest codepoint boundary and decodes
  from there.
- **Byte indexing:** `s.bytes[i]` returns a raw `byte` at byte offset `i`. This
  is O(1) always but returns raw UTF-8 bytes, not characters.
- **Iteration:** `for ch in s` yields `char` values (decoded codepoints)
  sequentially.
...
- Emoji: valid. `var speed: f64 = 0.0` compiles.
...
- Emoji with ZWJ sequences (e.g., family emoji): multiple codepoints, multiple
  elements. `len()` returns the codepoint count, not the visual glyph count.
...
- **Encoding detection**: Kairo does not detect or guess source file encoding.
  UTF-8, no BOM, period.
- **Null termination**: `string` is not null-terminated internally. Conversion
  methods for C interop add a null byte. Passing raw string data directly to a
  C function expecting a null-terminated string is undefined behavior.
